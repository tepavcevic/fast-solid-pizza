...
export const increaseCartItemQuantity = (id: number) => {
    ...
    cart.set(cart.get().map((cartItem) => {
...
export const getTotalCartQuantity = computed(cart, (cart) => cart.reduce((acc, curr) => acc + curr.quantity, 0))
export const getTotalCartPrice = computed(cart, (cart) => cart.reduce((acc, curr) => acc + curr.totalPrice, 0))
export const getCurrentQuantityById = (id: number) => computed(cart, (cart) => cart.find((item) => item.pizzaId === id)?.quantity ?? 0)